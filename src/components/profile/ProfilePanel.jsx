...
import { getFinancialProfile, createFinancialProfile, updateFinancialProfile } from '../../services/FinancialProfileService';
...
// Máscara de moeda (BRL) — mesmo padrão usado no formulário de transações
...
export default function ProfilePanel({ onClose, initialProfile, onProfileUpdate }) {
  ...
  const [finSalary,  setFinSalary]  = useState('');
  ...
  // Busca o perfil financeiro do usuário logado (carrega os campos se já existir)
  useEffect(() => {
    getFinancialProfile()
      .then(data => {
        setFinancialProfile(data);
        setFinName(data?.name ?? '');
        setFinSalary(data?.salaryRange ?? '');
      })
      .catch(() => {
        // Sem perfil financeiro cadastrado ainda — campos permanecem vazios
      });
  }, []);
  ...
  const handleSaveFinancial = async () => {
    if (!finName.trim())   { setFinError('O perfil financeiro é obrigatório.'); return; }
    if (!finSalary.trim()) { setFinError('A faixa salarial é obrigatória.'); return; }
    setFinSaving(true);
    setFinError('');
    setFinSuccess(false);
    try {
      const exists  = !!financialProfile;
      const payload = { name: finName.trim(), salaryRange: finSalary.trim() };
      const res     = exists
        ? await updateFinancialProfile(payload)
        : await createFinancialProfile(payload);

      // POST devolve { message, data }; PUT devolve apenas { message }
      const saved = res?.data ?? { ...financialProfile, ...payload };
      setFinancialProfile(saved);
      setFinName(saved.name ?? payload.name);
      setFinSalary(saved.salaryRange ?? payload.salaryRange);
      setFinSuccess(true);
      setFinMessage(res?.message ?? (exists ? 'Perfil financeiro atualizado com sucesso.' : 'Perfil financeiro criado com sucesso.'));
      setTimeout(() => setFinSuccess(false), 3000);
    } catch (err) {
      setFinError(err.response?.data?.message ?? 'Erro ao salvar.');
    } finally {
      setFinSaving(false);
    }
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
          ...
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              ...
              {financialProfile?.name && (
                <p className="text-sm mt-1" style={{ color: '#67e8f9' }}>
                  Perfil Financeiro: <span className="font-medium text-cyan-200">{financialProfile.name}</span>
                </p>
              )}
          ...
          {/* Perfil Financeiro */}
          <div className="space-y-4">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Perfil financeiro</h3>

            <div className="space-y-1.5">
              <label className="block text-xs text-slate-500">
                Perfil Financeiro <span className="text-red-400">*</span>
              </label>
              <input
                type="text"
                value={finName}
                placeholder="Ex: Investidor Conservador"
                onChange={e => { setFinName(e.target.value); setFinError(''); setFinSuccess(false); }}
                className="w-full rounded-xl px-4 py-3 text-sm text-slate-100 placeholder-slate-600 outline-none transition-all duration-200"
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={finSalary}
                  placeholder="0,00"
                  onChange={e => { setFinSalary(formatCurrencyInput(e.target.value)); setFinError(''); setFinSuccess(false); }}
                  className="w-full rounded-xl pl-10 pr-4 py-3 text-sm text-slate-100 placeholder-slate-600 outline-none transition-all duration-200"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', boxShadow: 'none' }}
                  onFocus={e => { e.target.style.borderColor = 'rgba(0,212,255,0.4)'; e.target.style.boxShadow = '0 0 0 3px rgba(0,212,255,0.08)'; }}
                  onBlur={e  => { e.target.style.borderColor = 'rgba(255,255,255,0.08)'; e.target.style.boxShadow = 'none'; }}
                />
            ...
            <button
              ...
              className="w-full py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
              style={{
                background: finSuccess ? 'rgba(34,197,94,0.18)' : 'rgba(0,212,255,0.12)',
                border:     finSuccess ? '1px solid rgba(34,197,94,0.4)' : '1px solid rgba(0,212,255,0.25)',
                color:      finSuccess ? '#86efac' : '#a5f3fc',
              }}
            >
              {finSaving ? (
                <span className="flex items-center justify-center gap-2">
              ...
              ) : (financialProfile ? 'Atualizar Perfil Financeiro' : 'Salvar Perfil Financeiro')}
            </button>
          </div>
